...
const contractaddress = "0xF73f83cE1B4a80d575Cb5cb4CFfeDFAeA0D2573A";
const abi = [
    {
      "inputs": [],
...
function home() {
    console.log("helow")
    var formContainer = document.getElementById("logo-box");
...
async function showAddress() {
    try {
        ...
        if(accounts.length > 0) {
            ...
            messageElement.textContent = account;
            navbar.insertBefore(messageElement, navbar.childNodes[2]);
        }    
        else {
...
document.getElementById('createPropertyForm').addEventListener('submit' , async (event) => {
    ...
    async function getPropertynow(price=null, sale=null) {
        ...
        for(let i=0; i< totalproperties; i++)
        {
            ...
            console.log("shdsf")
            const isauction = await contract.isAuctionStarted(i);
            ...
            if(property.forSale) {
                ...
                if(price == null)
                {
                    ...
                    Auction: ${isauction ? 'Started' : 'Not Yet'}<br>
                    <br><br>
                    `;
                }
                else
    ...
    async function yourProperty() {
        
        console.log("portfolio")
        var listContainer2 = document.getElementById("list-container2");
        // listContainer2.classList.toggle("hidden");

        let powner;
        const button = document.getElementById('getownedProperty');
        const yp = document.getElementById('yourPropertiesSection')

        const isVisible = !listContainer2.classList.contains('hidden');
        if (isVisible) {
            listContainer2.classList.add('hidden');
          } else {
            listContainer2.classList.remove('hidden');
          }
